...
Helper::prepare_input_files({
    ...
    copy => [ qw(ubr12224_0001.tif ubr12224.pdf  log4perl.conf) ],
});
...
my $pdf = Remedi::Singlepage::PDF::FromPDF->new(
    ...
    image => Remedi::Imagefile->new(
        ...
        isil => 'DE-355',
        file => path($Bin, qw(input_files ubr12224_0001.tif)),
    ...
    source_pdf  => Remedi::PDF::API2->open(file =>
        path($Bin, qw(input_files ubr12224.pdf))->stringify
    ),
    title => 'Titel',
    page_num    => 1,
    pages_total => 3,
...
is($pdf->colortype,'grayscale','colortype');
...
is($pdf->logo_file,
   path($Bin)->parent->child(
        qw( logo de-355 UBRLogo_grau_25_7.pdf ) ), 'logo_file',
);
my @box = (0,0,279.36,299.202519685039);
...
is($pdf->filestem,'ubr12224_0001','filestem');
